...
<main class="container">
    <main class="container">
        <!-- leads table -->
        <h2 class="display-4 page-header">Manage Users</h2>
        <p>You can edit users by clicking the edit icon.</p>
        <p>Add user using the Add user button, Search for users using the search bar</p>
        <div class="row">
        ...
        <table class="table my-5">
            ...
            if (isset($_GET['submit-search'])) {
                ...
                if (empty($search)) {
                    ...
                    $search_term = "%" . $search . "%";
                    $stm = $pdo->prepare("SELECT * FROM `users` WHERE (`name` LIKE ? OR `email` LIKE ? OR `username` LIKE ?)");
